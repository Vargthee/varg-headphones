...
export const ParallaxLayers = ({ scrollYProgress }: ParallaxLayerProps) => {
  ...
  const smoothProgress = useSpring(scrollYProgress, {
    stiffness: isLowPerf ? 30 : 50,
    damping: isLowPerf ? 30 : 20,
  });
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      >
        <div className={`absolute top-1/4 left-1/4 ${isMobile ? 'w-64 h-64' : 'w-96 h-96'} rounded-full bg-gradient-radial from-varg-white/10 to-transparent blur-3xl`} />
        {!isLowPerf && (
      ...
      {/* Foreground subtle grid - skip on mobile */}
      {!isMobile && (
        <motion.div
          className="absolute inset-0"
          style={{
